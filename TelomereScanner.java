...
public class TelomereScanner {
	 ...
	 * 
	 * Example input: java -jar TelomereCounter.jar TTAGGG 6 /path/to/readFile.txt.gz /path/to/Output_directory/
	 *
	 */
	public static void main(String[] args) throws Exception {
		...
		try (GZIPInputStream gzip = new GZIPInputStream(new FileInputStream(args[2]))) {
			...
			while ((line = br.readLine()) != null) {
				...
				long pair1TelomereCount = 0;
				long pair2TelomereCount = 0;
				long readTTAGGGPair1 = StringUtils.countMatches(pairs[0], motif);
				long readTTAGGGPair2 = StringUtils.countMatches(pairs[1], motif);
				long readCCCTAAPair1 = StringUtils.countMatches(pairs[0], oppositeMotif);
				long readCCCTAAPair2 = StringUtils.countMatches(pairs[1], oppositeMotif);
				long readTTAGGG = readTTAGGGPair1 + readTTAGGGPair2;
				long readCCCTAA = readCCCTAAPair1 + readCCCTAAPair2;
			...
			for (Map.Entry<TelomereMarker, Long> entry : telomereMap.entrySet()) {
				...
				if (!markerEntry.getCytosine()) {
					...
					long pair1Count = markerEntry.getPair1Count();
					long pair2Count = markerEntry.getPair2Count();
					long totalCount = (pair1Count+pair2Count);
					if (pair1Count >= pairTelomericRepeat && pair2Count >= pairTelomericRepeat && totalCount >= telomericRepeat) {
						totalTelomere += (totalCount) * entry.getValue();
					}
				} else {
			...
			try (Writer out = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(file), "utf-8"))) {
				out.write(String.format("%20s%n %20s%n %20s%n %20s%n %20s%n %20s%n %20s%n %20s%n", "ID: " + id.replace(".txt", ""),
					...
					(float)((float)(guanineCount/70)/(numberOfReads)), "Telomeres with repeats >=" + telomericRepeat + ": " + totalTelomere));
			...
			System.out.println("Analysis of " + id + " is complete. Telomeres with repeats >=" + telomericRepeat + ": " + totalTelomere);
		} catch (IOException e) {
